...
// memo[tak][i][j]: max score of tak in field[i][j]
// memo[aok][i][j]: min score of aok in field[i][j]
...
int rec_tak(int i, int j) {
  ...
  return memo[tak][i][j] = max(player_right_score, player_down_score);
}

int rec_aok(int i, int j) {
  if (memo[aok][i][j] != EMPTY) return memo[aok][i][j];

  bool can_go_right = good2go(i, j + 1);
  bool can_go_down = good2go(i + 1, j);

  if (!can_go_down && !can_go_right) {
    return memo[aok][i][j] = 0;
  }
  ...
  return memo[aok][i][j] = min(player_right_score, player_down_score);
}
